<template>
  <view>
	...
	<view class="tabbar container30"><top-bar1 /></view>
	<!-- 主页面 -->
	<view class="content1">
		<!-- slider -->
		...
		<view class="new-plate">
			<view class="container">
				<view class="title text-color flex-bet ">
					...
					<view class="right flex-center font24">{{ currentTab ? '更多新歌' : ' ' }}</view>
				</view>
...
export default {
	...
	data() {
		return {
			...
			// topbar: [{ name: '我的' }, { name: '主页' }, { name: '视频' }, { name: '其它' }],
	...
	methods: {
		...
		openDrawer() {
		      this.$emit('openDrawer'); // 触发一个自定义事件，通知父组件打开抽屉
		    },
		    clickTab(index) {
		      this.current = index; // 设置当前激活的tab
		      this.$emit('clickTab', index); // 触发一个自定义事件，通知父组件tab点击事件
		    },
		async getNewSong() {
			// 获取新歌
